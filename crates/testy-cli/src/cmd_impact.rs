    ...
    match args.command {
        ...
        None => {
            crate::cmd_common::maybe_warn_deprecated(
                "warning: `testy impact` legacy mode is deprecated; use `testy impact plan` and `testy impact run`.",
            );
...
const IMPACT_PLAN_EXAMPLE: &str = r#"{
  ...
  "next_command": "testy impact run --plan plan.json -- <your-test-command-template>"
}"#;
...
fn run_impact_run(args: ImpactRunArgs) -> Result<i32> {
    ...
    let plan_path = args.plan.as_deref().ok_or_else(|| {
        anyhow::anyhow!("Missing --plan. Use: testy impact run --plan plan.json -- <command>")
    })?;

    if args.command.is_empty() {
        anyhow::bail!(
            "No command template provided. Use: testy impact run --plan plan.json -- <command>"
        );
    }
